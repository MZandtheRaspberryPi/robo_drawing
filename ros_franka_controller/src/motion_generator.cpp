...
MotionGenerator::MotionGenerator(double speed_factor, const std::array<double, 7> q_goal, RobotJointState *out_state, std::mutex *state_mutex)
    : q_goal_(q_goal.data())
{
                                                   ...
                                                   franka::Duration period)
{
    ...
    output.motion_finished = motion_finished;
    {
        ...
        {
            out_state_->pos[i] = robot_state.q[i];
            out_state_->vel[i] = robot_state.dq[i];
            out_state_->tau[i] = robot_state.tau_J[i];
        }
    }
